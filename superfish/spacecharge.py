...
def routine(
    ...
    if main_conf.DEBUG:
        print(f"== Lab Frame ==")
        print(f"Sample Three-Velocity: {alt_parts[2].get_3v()}")
        print(f"Ref. Part. Three-Velocity: {lab_ref_vel}")
        print(f"Ref. Part. Three-Position {lab_ref_pos}")
        print(f"Gamma Factor: {fv.gamma_3v(lab_ref_vel)}")
        print(f"Bunch Charge: {bunch_charge}")
        print(f"Bunch Length: {lab_bunch_len}")
    ...
    if main_conf.DEBUG:
        print(f"== Com Frame ==")
        print(f"Com. Ref. Part. Three-Position: {reference.get_3p()}")
    ...
    ###
    # STEP 5: Integrate using Jefimenko's Equation
    ###
    print("cml:", com_bunch_len)
    com_efld_cyl = ltsolvers.solve_SCFields(
        bunch_rad=bunch_rad_max,
    ...
    if main_conf.SHOW_FIELD_PLOTS:
        idx = closestVal(bunch_rad_max, com_x_mesh) #TODO: Add something for user to set the place where we are looking at
    ...
    #         then convert that into the lab frame using the SR functions that I defined.
    ###
    ...
    # We want to interpolate the field solves so evaluating them at particle positions
    # is easy.
    ...
    for E_com in E_com_at_particles:
        E_lab, B_lab = fv.inverse_field_transform(E_com, np.zeros(3), lab_ref_vel)
        lab_E_at_particles.append(E_lab)
        lab_B_at_particles.append(B_lab)

    lab_E_at_particles = np.array(lab_E_at_particles)

    print(f"labpart: {lab_E_at_particles[1]}")

    lab_B_at_particles = np.array(lab_B_at_particles)

    if main_conf.DEBUG:
        print(f"== Lab Frame ==")
        print(f"Sample E: {lab_E_at_particles[1]}")
        print(f"Center E: {Ex_interp([0,0,0])}, {Ey_interp([0,0,0])}, {Ez_interp([0,0,0])}")
        print(f"Sample B: {lab_B_at_particles[1]}")
        print("[ END CYCLE ] \n \n")

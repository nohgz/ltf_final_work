...
def field_transform(efld, bfld, vel_3v):
    ...
    vel_3v_hat = vel_3v / norm(vel_3v)
    e_term1 = gamma_3v(vel_3v) * (efld + np.cross(vel_3v, bfld))
    e_term2 = (gamma_3v(vel_3v) - 1) * np.dot(efld, vel_3v_hat) * vel_3v_hat
    eprime = e_term1 - e_term2

    b_term1 = gamma_3v(vel_3v) * (bfld - np.cross(vel_3v, efld)/9E16)
    b_term2 = (gamma_3v(vel_3v) - 1) * np.dot(bfld, vel_3v_hat) * vel_3v_hat
    bprime = b_term1 - b_term2

    return eprime, bprime

def inverse_field_transform(efld, bfld, vel_3v):
    return field_transform(efld, bfld, np.array(vel_3v) * -1)

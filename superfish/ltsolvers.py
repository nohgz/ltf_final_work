...
def _jit_integrate(
    ...
    """
    Integrator for the longitudinal and transverse electric fields generated by a bunch.
    Integrates Jefimenko's equation via trapezoid rule or gaussian quadrature.
    """
    ...
    # print("SIDUFS", b_s - a_s, b_z - a_z, b_phi - a_phi)
    ...
    for i in prange(n):
        ...
        for j in range(n):
            ...
            for k in range(n):
                ...
                # integrand = s * rho
                integrand = (roots[i] + 1) * rho * separation / (np.linalg.norm(separation) ** 3 * np.pi * bunch_rad * 4.5)
                result += ws * wz * wphi * integrand

    # Scale factor
    if is_trapezoid:
        int_factor = ds * dz * dphi
    else:
        int_factor = 1/16 * ((b_s - a_s) * (b_z - a_z) * (b_phi - a_phi))

    return bunch_charge * COULOMB_K * int_factor * result
